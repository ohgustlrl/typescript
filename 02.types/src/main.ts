...
let float : number = 1.12345

// 3. String 타입

let string : string
let firstName = "Doe"
let firstName2:string = "Doe"

// 4. Array 타입
// 한가지 타입만 가지는 배열

let arr1: string[] = ["john", "kim"]
let arr2: Array<string> = ["john","kim"]  //인터페이스 Array에 타입을 준 방법, 위와 같음

// 여러 타입을 가지는 배열 (유니언 타입 사용)

let arr3: (string | number)[] = ["John", 1, 2]
let arr4: Array<string | number> = ["John", 1, 2]
...
let users: [number, string][] // 2차원 배열을 의미
users = [[1, "john"], [2, "Doe"]]
...
let string2: boolean = unknown as boolean // 해당 형태는 약간 강제로 타입을 지정하는 형태로 강제 지정하는 방법 중 하나
...
const obj1:{id: number, title: string} = { // 객체에는 속성별로 {}를 통해 타입을 지정할 수 있음
  id: 1,
  title : "타이틀1"
}
...
let func1: (arg1: number, arg2: number ) => number // (argument : type ) => 반환하는 값에 대한 type
func1 = function(x, y) {
  return x * y
}
...
let func2: () => void;  // 반환되는 값이 없는 경우 () => void 타입을 지정
func2 = function() {
  console.log('hi')
}
...
let void2:void = undefined; // void 타입에는 undefined는 할당 가능함
